...
This part is for basic time series analysis on Quarterly GDP data **(source:IMF)**. It shows quarterly Nominal Gross Domestic Product seasonally Un-adjusted calculated in domestic currency (value in Million) for the period 2nd Quarter 2004 to 3rd Quarter 2022.
...
```{r ts_gdp_data1_plot, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Fig 1. India quarterly GDP data in Domestic Currency"}
...
```{r basic_lm_graph, fig.cap="Fig 2. India Quarterly GDP data and linear trend", fig.align='center'}
options(scipen = 100, digits = 4)
plot(ts_gdp_data1,
...
Figure 2 shows the linear trend and actual quarterly India Nominal GDP values. One of the important assumption for linear regression is **Residuals Distributed Normally**. It is also to know the residuals are random or not, which is essential for explaining relation in the data.
...
```{r autocorrelation}
options(scipen = 100, digits = 4)
acf(ts_gdp_data1, main = 'Correlogram of GDP')
...
detrend_gdp = diff(ts_gdp_data1)

plot(detrend_gdp, main = 'Detrend GDP dataset', ylab = 'GDP', xlab = 'Quarter', col = 'blue', lwd = 2)

```
Correlogram after detrend the data,
```{r detrended acf}
options(scipen = 100, digits = 4)
acf(detrend_gdp, main = 'Correlogram of Detrended GDP')
